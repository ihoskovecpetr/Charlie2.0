...
export default function SocialLogins() {
  ...
  var GoogleV =
    "https://res.cloudinary.com/party-images-app/image/upload/v1557462915/vgvduspc6s8j368y1ras.png";
  ...
  //After fbc/google approval, login or make new user
  useEffect(() => {
      ...
      if (data.loginExternal.dataOut.success) {
        ...
        if (data.loginExternal.dataOut.typeSocial) {
          window.localStorage.setItem(
            "token",
            data.loginExternal.dataOut.token
          );
          setContext((prev) => {
            return {
              ...prev,
  ...
  useEffect(() => {
    ...
    if (
      newUserStates &&
      newUserStates.data &&
      newUserStates.data.newUser &&
      newUserStates.data.newUser.dataOut
    ) {
      if (newUserStates.data.newUser.dataOut.success) {
        window.localStorage.setItem(
          "token",
          newUserStates.data.newUser.dataOut.token
        );
        setContext((prev) => {
          return {
            ...prev,
            ...
            telephone: newUserStates.data.newUser.dataOut.telephone,

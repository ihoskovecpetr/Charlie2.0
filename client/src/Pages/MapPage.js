...
function MapPage(props) {
  ...
  const [mapOptions, setMapOptions] = useState({
    center:
      props.workingPosition && props.workingPosition.geometry
        ? props.workingPosition.geometry
        : { lat: 50.068645, lng: 14.457364 }, // ,
    zoom:
      props.workingPosition && props.workingPosition.zoom
        ? props.workingPosition.zoom
        : 10,
  ...
  const { loading, error, data, fetchMore } = useQuery(ALL_EVENTS, {
    variables: {
      date:
        props.workingPosition && props.workingPosition.date
          ? props.workingPosition.date
    ...
    });
    {
      if (!loading) {
        AllMarkersArr = UniqArr.map((location, i) => {
          var urlNA =
            "https://res.cloudinary.com/party-images-app/image/upload/v1583692669/ijy8dorejsarudzkauma.png"; // {BasicMarker}
          var urlAttend =
